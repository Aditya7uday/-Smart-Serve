...
import { LayoutDashboard, UtensilsCrossed, ClipboardList, PackageSearch, Users, CreditCard, MessageSquare, Bike, LogOut, UserCircle, X } from 'lucide-react';
...
export function Sidebar({ role, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`bg-white shadow-sm flex flex-col border-r border-gray-100 transition-all duration-300 ease-in-out ${
          isOpen
            ? 'fixed inset-y-0 left-0 w-64 h-full z-50'
            : 'hidden'
        } md:flex md:sticky md:top-0 md:h-screen md:w-64 md:z-30 flex-shrink-0`}
      >
        <div className="p-6 flex items-center justify-between">
          ...
          <button 
            onClick={onClose} 
            className="md:hidden p-1.5 text-secondary-gray hover:text-dark-text rounded-lg transition-colors"
